...
coicop_set_hierarchy$level <- ifelse(
        coicop_set_hierarchy$coicop18_code == "CP00",
        1,
        nchar(gsub("\\D", "", coicop_set_hierarchy$coicop18_code))
)
coicop_set_hierarchy$parent_code <- ifelse(
        coicop_set_hierarchy$level > 2,
        substr(coicop_set_hierarchy$coicop18_code, 1, nchar(coicop_set_hierarchy$coicop18_code) - 1),
        NA
)
coicop_set_hierarchy$parent_code <- ifelse(
        coicop_set_hierarchy$level == 2 & coicop_set_hierarchy$coicop18_code != "CP00",
        "CP00",
        ifelse(
                coicop_set_hierarchy$level > 2,
                substr(coicop_set_hierarchy$coicop18_code, 1, nchar(coicop_set_hierarchy$coicop18_code) - 1),
...
# Update parent_code for Eurostat codes where a zero is part of the parent-child structure.
...
# ------------------------------------------------------------
# Administered prices hierarchy
# ------------------------------------------------------------

ap_hierarchy <- data.frame(
        coicop18_code = c(
                "APF",
                "APM",
                "TOT_X_AP",
        ...
        coicop18 = c(
                "Fully administered prices",
                "Mainly administered prices",
                "Overall index excluding administered prices",
                "Total"
        ),
        level = c(2, 2, 2, 1),
        parent_code = c("CP00", "CP00", "CP00", NA_character_),
        stringsAsFactors = FALSE
)

ap_hierarchy$code_label <- paste(
        ap_hierarchy$coicop18_code,
        ap_hierarchy$coicop18,
        sep = " "
)
...
label_set_ap <- select(ap_hierarchy, coicop18_code, code_label)
